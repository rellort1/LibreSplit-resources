...
local route = {
    prologue = { "Clear" },
    forsakenCity = { "6", "9b", "Clear" },
    oldSite = { "3", "end_0", "Clear" },
    celestialResort = { "08-a", "09-d", "00-d", "Clear" },
...
function split()
    ...
    elseif current.levelName ~= old.levelName then
        if current.levelName == route[Chapter][checkpointIndex] then
            shouldSplit = true
            checkpointIndex = checkpointIndex + 1
        end
    end
